          ...
          <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
            <div class="row">
                ...
                <div class="card-body px-4 pb-2">
                    ...
                    <hr />
                    <form role="form" method="POST" action="{{ route('agent-management.update', $agent->id) }}"
                        ...
                        <div class="row">
                            <div class="form-group col-12 p-3">
                                ...
                                <select name="user_id" class="select2-multiple form-control fs-14  h-50px" required>
                                    @foreach ($user as $row)
                                        <option value="{{ $row->id }}"
                                            @if ($row->id == $agent->user_id) selected @endif>{{ $row->name }}
                                            ({{ $row->email }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-12 p-3">
                                ...
                                <select name="town_id" class="select2-multiple form-control fs-14  h-50px" required>
                                    @foreach ($town as $row)
                                        <option value="{{ $row->id }}"
                                            @if ($row->id == $agent->town_id) selected @endif>{{ $row->town }} </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-12 p-3">
                            ...
                            <div class="form-group col-12 p-3">
                                ...
                                <select name="type_id" class="select2-multiple form-control fs-14  h-50px" required>
                                    @foreach ($type as $row)
                                        <option value="{{ $row->id }}"
                                            @if ($row->id == $agent->type_id) selected @endif>({{ $row->title }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
